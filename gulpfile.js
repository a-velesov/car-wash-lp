...
const gulp = require('gulp'), //господин гульп
    ...
    rigger = require('gulp-rigger'); //инклудилка для файлов (https://www.npmjs.com/package/gulp-rigger)

sass.compiler = require('node-sass'); //компилятор для scss

const path = { //пути до всего в одном месте
...
const serverParams = { //параметры нашего сервера
    root: path.base,
    port: 9000,
    keepalive: true,
    livereload: true
...
/*gulp.task('app', function(){
    let options = {
      uri: 'localhost:'+serverParams.port
    };
...
gulp.task('sass', function(){
    return gulp.src(path.src.sass) // собираем sass
